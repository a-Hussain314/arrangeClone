...
import { View, Text, Image, TouchableOpacity, Platform, FlatList, ActivityIndicator } from 'react-native';
...
export default function Notification({ navigation }) {
            ...
            .then(res => {
                if (res.data.status === 1) {
                    ...
                    if (pageNo == 1) {
                        ...
                        data && data.list ? setNotificationList([...notificationList, ...data.list]) : setNotificationList([...notificationList]);
                    }
                } else {
                    setLoading(false);
                    setRefreshing(false);
    ...
    const _notificationList = (item, index) => {
        return (
            <TouchableOpacity onPress={() => { }} style={{ marginBottom: 10, borderRadius: 10, borderColor: 'rgba(0,0,0, 0.2)', borderBottomColor: colors.themeColor, padding: 10, borderWidth: 0.5, paddingHorizontal: 20, flexDirection: 'row', }}>
