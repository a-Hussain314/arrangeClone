...
export default function ProviderProfileScreen({ route, navigation }) {
  ...
  const [priceHideShow, setPriceHideShow] = React.useState(false);
  ...
  const [timeManage, setTimeManage] = React.useState([
    { open: '', close: '' },
    { open: '', close: '' },
    { open: '', close: '' },
    { open: '', close: '' },
  ...
  const [openTimeForPicker, setOpenTimeForPicker] = React.useState([
    '09:00 AM',
    '10:00 AM',
    '11:00 AM',
    '12:00 PM',
  ...
  const [closeTimeForPicker, setCloseTimeForPicker] = React.useState([
    '09:00 AM',
    '10:00 AM',
    '11:00 AM',
    '12:00 PM',
    ...
    getService('profile/detail')
      .then((res) => {
        if (res.data.status === 1) {
          ...
          setPrice(data.cancellation > 0 ? data.cancellation : '');
          setPriceHideShow(false);
  ...
  const selectOpenTime = async (time, index, i) => {
    ...
    if (timeManage[i].closeVal <= time.val) {
      showDangerToast(I18n.t("err_msg_fromTime_to_endTime"));
      return;
    }
  ...
  const selectCloseTime = async (time, index, i) => {
    ...
    if (timeManage[i].open == '') {
      showDangerToast(I18n.t("err_select_first_time"));
      return;
    }
  ...
  const _onSelect = (item, index) => {
    ...
    const arr2 = [...timeManage];
    arr2[index].select = !arr2[index].select;
    if (arr2[index].select == true) {
      arr2[index].open = '09:00 AM';
      arr2[index].close = '09:00 PM';
    } else {
      arr2[index].open = '';
      arr2[index].close = '';
    }
  ...
  const handleFromPress = (key) => {
    const timeArr = [...timeManage];
    ...
    Picker.init({
      ...pickerStyle,
      pickerData: openTimeForPicker,
      selectedValue: [
        `${timeArr[key].open ? timeArr[key].open : openTimeForPicker[0]}`,
      ],
      pickerTextEllipsisLen: 25,
      pickerCancelBtnText: I18n.t('lbl_cancel'),
      pickerConfirmBtnText: I18n.t('lbl_confirm'),
      pickerTitleText: I18n.t('lbl_select_first_time'),
      onPickerConfirm: async (data) => {

        await openTime.map((itm) => {
          if (data[0] == itm.name) {
            timeManage[key].opneVal = itm.val;
          }
        });

        await closeTime.map((itm) => {
          if (timeManage[key].close == itm.name) {
            timeManage[key].closeVal = itm.val;
          }
        });

        if (timeManage[key].closeVal <= timeManage[key].opneVal) {
          showDangerToast(I18n.t('err_msg_fromTime_to_endTime'));
          return;
        }

        timeArr[key].open = data[0];
        setTimeManage(timeArr);
      },
      onPickerCancel: (data) => { },
      onPickerSelect: (data) => { },
    });
  ...
  const handleToPress = (key) => {
    ...
    Picker.init({
      ...pickerStyle,
      pickerData: closeTimeForPicker,
      selectedValue: [
        `${timeArr[key].close ? timeArr[key].close : closeTimeForPicker[0]}`,
      ],
      pickerTextEllipsisLen: 25,
      pickerCancelBtnText: I18n.t('lbl_cancel'),
      pickerConfirmBtnText: I18n.t('lbl_confirm'),
      pickerTitleText: I18n.t('lbl_select_endTime'),
      onPickerConfirm: async (data) => {

        if (timeManage[key].open == '') {
          showDangerToast(I18n.t("err_select_first_time"));
          return;
        }

        await openTime.map((itm) => {
          if (timeManage[key].open == itm.name) {
            timeManage[key].opneVal = itm.val;
          }
        });
        ...
        if (timeManage[key].opneVal >= timeManage[key].closeVal) {
          showDangerToast(I18n.t("err_endTime_not_greator"));
          return;
        }

        timeArr[key].close = data[0];
        setTimeManage(timeArr);
      },
      onPickerCancel: (data) => { },
      onPickerSelect: (data) => { },
        ...
        <View style={{ flex: 0.25, flexDirection: 'row' }}>
          <View style={{ flex: 0.3, justifyContent: 'center' }}>
            {timeManage[i].select || timeManage[i].open ? (
              <Image
                source={globalImagePath.greenCheckmark}
                resizeMode={'contain'}
        ...
        {timeManage[i].select || timeManage[i].open ? (
            ...
            <View style={{ flex: 1 }}>
              <View style={{}}>
                <TouchableOpacity
                  ...
                  onPress={() => handleFromPress(i)}>
                  <Text style={{ marginLeft: 10 }}>
                    {timeManage[i].open ? timeManage[i].open : 'From Time'}
                  </Text>
                </TouchableOpacity>

              </View>
              {timeManageOpenError[i].length > 0 && (
                <ErrorMessage text={timeManageOpenError[i]} />
              )}
            </View>
            ...
            <View style={{ flex: 1 }}>
              <View style={{}}>
                <TouchableOpacity
                  ...
                  onPress={() => handleToPress(i)}>
                  <Text style={{ marginLeft: 10 }}>
                    {timeManage[i].close ? timeManage[i].close : 'End Time'}
                  </Text>
                </TouchableOpacity>

              </View>
              {timeManageCloseError[i].length > 0 && (
                <ErrorMessage text={timeManageCloseError[i]} />
              )}
            </View>
          </View>
        ) : (
          <View
            ...
            <View style={{ justifyContent: 'center' }}>
              <Text
                style={
                  (CommonStyles.TextInputStyle(14, 50),
  ...
  const showActionSheet = () => {
    return (
      <ActionSheet
        ...
        title={''}
        options={[I18n.t('lbl_camera'), I18n.t('lbl_gallery'), I18n.t('lbl_cancel')]}
        cancelButtonIndex={2}
          ...
          <View style={{ width: '100%', marginTop: normalize(30) }}>
            <TextInput
              ...
              onChangeText={(salonName) => {
                ...
                setSalonNameError(validate('salonName', salonName, '', true));
              }}
              value={salonName}
            />
            ...
            <View style={{}}>
              ...
              <View
                ...
                <RadioButton
                  ...
                  onPress={() => {
                    selectService(1), Keyboard.dismiss();
                  }}
                />
            ...
            {isChecked ? (
              priceHideShow > 0 ?
                <TextInput
                  ...
                  onChangeText={(price) => {
                    setPrice(price);
                    setPriceError(validate('price', price, '', true));
                  }}
                  value={`${price}`}
                />
                :
                <View>
                  <View style={{ flexDirection: 'row' }}>
                    <Text style={{ ...CommonStyles.InputLabelStyle() }}>{I18n.t('lbl_cancel_charge')}</Text>
                  </View>
                  <TouchableOpacity style={{ height: width * (55 / 375), justifyContent: 'center', borderColor: 'rgb(196,170,153)', borderBottomWidth: 0.5, }} onPress={() => { setPriceHideShow(true) }}>
                    <Text style={CommonStyles.TextInputStyle(14), { paddingTop: 8, }}>{`${price}` + '%'}</Text>
                  </TouchableOpacity>
                </View>
            ) : null}
              ...
              <View>
                <View style={{ flex: 1, marginTop: 15 }}>
                  <FlatList
                    ...
                    data={timeSlot}
                    renderItem={({ item, index }) => renderTimeSlots(item, index)}
