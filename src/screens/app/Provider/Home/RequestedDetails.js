...
export default function RequestDetails({navigation, route}) {
  ...
  const [employeeAlreadyAssigned, setEmployeeAlreadyAssigned] = React.useState(
    false,
  );
  const [employee, setEmployee] = React.useState('');
  const [employeeID, setEmployeeID] = React.useState('');
  const [employeeArr, setEmployeeArr] = React.useState([]);
  const [employeeFullArr, setEmployeeFullArr] = React.useState([]);

  React.useEffect(() => {
    AsyncStorage.getItem('user', (err1, item1) => {
      var userDetail = JSON.parse(item1);
      if (route.params.requestType) {
        setType(route.params.requestType);
        setBookingId(route.params.bookingId);
        setEmployeeAlreadyAssigned(route?.params?.employeeID ? true : false);
        setEmployee(route.params.employeeName);
      ...
      .then(async (res) => {
        ...
        if (res.data.status === 1) {
          ...
          data.map((item) => {
            ...
            newArr1.push({id: item._id, name: item.name});
          });
          setEmployeeArr(await newArr);
          setEmployeeFullArr(await newArr1);
        } else {
          setLoading(false);
          setTimeout(function () {
            showDangerToast(res.data.message);
  ...
  const handleEmployeePress = () => {
    ...
    Picker.init({
      ...pickerStyle,
      pickerData: employeeArr,
      selectedValue: [`${employee ? employee : employeeArr[0]}`],
      pickerTextEllipsisLen: 25,
      pickerCancelBtnText: `${I18n.t('lbl_cancel')}`,
      pickerConfirmBtnText: `${I18n.t('lbl_confirm')}`,
      pickerTitleText: `${I18n.t('lbl_select_employee')}`,
      onPickerConfirm: (data) => {
        setEmployee(data[0]);

        employeeFullArr.map((item) => {
          if (item.name == data[0]) {
  ...
  const assignEmployee = () => {
    if (employee == '') {
      showDangerToast(`${I18n.t('lbl_please_select_employee_to_assign')}`);
      return;
    }
    ...
    setLoading(true);
    //***** api calling */
    postService('employee/asign-booking-to-employee', data)
      .then((res) => {
        if (res.data.status === 1) {
          showToast(res.data.message);
          setLoading(false);
          setEmployeeAlreadyAssigned(true);
          navigation.goBack();
        } else {
          setLoading(false);
          setTimeout(function () {
  ...
  const bookAcceptAndReject = (state) => {
    const data = {
  ...
  const ConfirmAlert = (state, type) => {
    Alert.alert(
      `Are you sure want to ${type} appointment`,
      '',
      [
        {
        ...
        {
          text: `${I18n.t('lbl_ok')}`,
          onPress: () => bookAcceptAndReject(state),
        },
      ],
      {cancelable: false},
    );
  ...
  const bookingCompleteAlert = (state) => {
    Alert.alert(
      `Are you sure want to complete appointment`,
      '',
      [
        {
          text: `${I18n.t('lbl_cancel')}`,
        ...
        {text: `${I18n.t('lbl_ok')}`, onPress: () => completeBooking(state)},
      ],
      {cancelable: false},
    );
  };
                      ...
                      <TouchableOpacity
                        onPress={() => {
                          navigation.navigate('TrackOrder', {
                            ...
                            employee_lat:
                              bookingDetails?.employeeData?.location
                                ?.coordinates[1],
                            employee_long:
                              bookingDetails?.employeeData?.location
                                ?.coordinates[0],
                        ...
                        style={styles.editOuter}>
                        <Text style={styles.btnText2}>{'Track Employee'}</Text>
                      </TouchableOpacity>
                    ) : null}
                  </View>
                  <View style={styles.completeOuter}>
                    {route.params.requestType == 'upcoming' &&
                    status != 5 &&
                    bookingDetails?.assign_details?.visit_completed_at ? (
                      <TouchableOpacity
                        onPress={() => {
                          bookingCompleteAlert(3);
                        }}
                        style={styles.editOuter}>
                        <Text style={styles.btnText2}>
                          {'Booking Complete'}
              ...
              <View style={styles.serviceTitleView}>
                <Text style={styles.serviceText}>
                  {employeeAlreadyAssigned
                    ? I18n.t('lbl_assigned_employee')
                    : I18n.t('lbl_assign_employee')}
                </Text>
              </View>
              <View style={styles.renderOuterView}>
                <View
                  ...
                  <View style={{flexDirection: 'row'}}>
                    <View style={{marginLeft: 10, marginTop: 2}}>
                      <View
                        ...
                        <View
                          style={{
                            width: employeeAlreadyAssigned ? '95%' : '70%',
                          }}>
                          <TouchableOpacity
                            style={{
                              ...
                              <View style={{flexDirection: 'row'}}>
                                <Text
                                  style={{
                                    ...
                                    fontSize: 14,
                                    flex: 1,
                                  }}>{`${I18n.t('lbl_select_employee')}`}</Text>
                                <View style={{justifyContent: 'center'}}>
                                  <Image source={globalImagePath.dropIcon} />
                                </View>
                              </View>
                        ...
                        {employeeAlreadyAssigned ? null : (
                          <View style={{width: '25%', height: 50}}>
                            <TouchableOpacity
                              ...
                              <Text
                                style={{...CommonStyles.buttonTextStyle(14)}}>
                                {I18n.t('lbl_assign')}
                              </Text>
                            </TouchableOpacity>
                          </View>
