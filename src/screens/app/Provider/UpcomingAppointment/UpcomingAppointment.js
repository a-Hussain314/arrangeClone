...
export default function UpcomingAppointment({navigation}) {
    ...
    postService('salon-appointment', data)
      .then((res) => {
        if (res.data.response) {
          console.log('appointment res.data =>', res.data.response);
          let data = res.data.response;
          // setUpcomingRequest(data);
          setProfileUrl(res.data.response.profileUrl);
          if (pageNo == 1) {
            setUpcomingRequest([...data.list]);
          } else {
            data && data.list
              ? setUpcomingRequest([...upcomingSalon, ...data.list])
              : setUpcomingRequest([...upcomingSalon]);
          }
          setRefreshing(false);
  ...
  const bookAcceptAndReject = (id, state, index) => {
    const data = {
      booking_id: id,
      status: state,
    };
    ...
    postService('salon-appointment/update-booking', data)
      .then((res) => {
        ...
        if (res.data.status === 1) {
          ...
          const newUpcomingList = [...upcomingSalon];
          newUpcomingList.splice(index, 1);
          setUpcomingRequest(newUpcomingList);
        } else {
          setLoading(false);
          setTimeout(function () {
            showDangerToast(res.data.message);
  ...
  const cancelBookingAlert = (id, state, index) => {
    Alert.alert(
      `${I18n.t('lbl_cancel_request')}`,
      '',
      [
        {
        ...
        {
          text: `${I18n.t('lbl_ok')}`,
          onPress: () => bookAcceptAndReject(id, state, index),
        },
      ],
      {cancelable: false},
    );
        ...
        <View
          ...
          <View>
            ...
            <View style={{flexDirection: 'row'}}>
              ...
              <TouchableOpacity
                onPress={() => {
                  cancelBookingAlert(item._id, 2, index);
                }}
