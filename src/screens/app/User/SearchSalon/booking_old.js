...
import { Text, View, FlatList, TouchableOpacity, StyleSheet, Image, ActivityIndicator, Alert, Keyboard, ScrollView, Modal } from 'react-native';
...
export default function Booking({ route, navigation }) {
    ...
    React.useEffect(() => {
        ...
        // console.log("route.params.booking_time =>", route.params.booking_time);
        Geolocation.getCurrentPosition(
            (position) => {
                setCurrentLat(position.coords.latitude);
                setCurrentLng(position.coords.longitude);
    ...
    const confirmRemoveAlert = (item, index) => {
        Alert.alert(
            ...
            [
                ...
                { text: 'OK', onPress: () => removeProduct(item, index) },
            ],
            { cancelable: false },
        );
    };
                        ...
                        {cashOptionAvailable <= 0 ?
                            <View style={{ marginLeft: 10 }}>
                                <RadioButton
                                    ...
                                    fontSize={14}
                                    titleColor={'rgb(25,27,28)'}
                                    onPress={() => { Keyboard.dismiss(); setPaymentMethod(2); setPaymentMethodError(''); }}
                                />
                            </View> : null}
                    </View>
    ...
    const _renderAddress = (item, index) => {
        return (
            <View
                ...
                <RadioButton
                    placeholder=""
                    title={item.tag_location == 1 ? 'Home' : item.tag_location == 2 ? 'Office' : 'Other'}
                    checkedIcon={globalImagePath.selectRadio}
                    uncheckedIcon={globalImagePath.nonSelectRadio}
                    marginRight={10}
                    fontSize={14}
    ...
    const addCard = () => {
        ...
        if (
            cardNumberError || cardNameError || monthError || yearError || cvvError
        ) {
        } else {
            setLoading(true)
            let data = {
    ...
    const selectedCardpayment = () => {
        ...
        postService('home/makeOrderPayment', data)
            .then(async res => {
                setLoading(false);
                console.log("add card res ==> ", res.data.response);
                if (res.data.status === 1) {
                    setLoading(false);
                    showToast(res.data.message);
                    setVisibleCardModal(false);
    ...
    const placeOrder = () => {
        ...
        if (serviceLocationError || paymentMethodError) {
            ...
            postService('home/bookslot', postData)
                .then(async res => {
                    ...
                    if (res.data.status === 1) {
                        ...
                        if (res.data.response.payment_type == 1) {
                            showToast(res.data.message);
                            setTimeout(() => {
                                setVisibleCardModal(true);
                            }, 5);

                        } else if (res.data.response.payment_type == 0) {
                            navigation.reset({
                                index: 0,
                                routes: [{
                                    name: 'Congratulation',
                        ...
                        } else if (res.data.response.payment_type == 2) {
                            navigation.reset({
                                index: 0,
                                routes: [{
                                    name: 'Congratulation',
                            ...
                            >
                                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <Text style={{
                                        ...
                                        fontSize: 18,
                                        fontFamily: fonts.type.NunitoSans_SemiBold,
                                    }}>{addressData.length >= 1 ? !isAddNewAddress ? 'Manage Address' : I18n.t('lbl_select_address') : I18n.t('lbl_select_address')}</Text>
                            ...
                            >
                                ...
                                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <Text style={{
                                        ...
                                        fontSize: 18,
                                        fontFamily: fonts.type.NunitoSans_SemiBold,
                                    }}>{isAddNewCard ? 'Enter Card Details' : cardData.length > 0 ? 'Manage Card' : 'Select From Cards'}</Text>
                ...
                <View style={styles.container}>
                    ...
                    <View style={{ padding: 20 }}>
                        ...
                        <View style={{ marginTop: 10, marginBottom: 40 }}>
                            {_rendetItems("Booking Date", route.params.booking_date, "booking")}
                            {_rendetItems("Booking Time", route.params.booking_time, "booking", route.params.bookingEndTime)}
